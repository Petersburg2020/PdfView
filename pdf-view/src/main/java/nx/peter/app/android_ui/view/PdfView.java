...
public class PdfView extends AView<PdfView> {
    ...
    protected void reset() {
        setPdfAssetFile("pdf/sample.pdf");
    }
    ...
    public void setPdfAssetFile(@NonNull CharSequence assetFile) {
        AssetManager asset = getContext().getAssets();
        try {
            if (asset.open(assetFile.toString()) == null) {
                PDFViewPagerZoom pager = new PDFViewPagerZoom(getContext(), assetFile.toString());
                setView(pager);
            }
        } catch (IOException e) {
            Log.e("PdfView", e.getMessage());
        }
    }
